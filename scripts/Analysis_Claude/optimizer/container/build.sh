...
DEF="optimizer.def"
# Defaults beside the recipe, in container/. That is where run_in_container.sh, t3opt_ceres.sh
# and submit.local.sh all look, and .gitignore excludes container/*.sif so the 1-2 GB binary is
# never committed. It does NOT belong in OPTIMIZER_HOME: that holds state you cannot
# regenerate, and an image is regenerated by re-running this script.
SIF="${1:-optimizer.sif}"
