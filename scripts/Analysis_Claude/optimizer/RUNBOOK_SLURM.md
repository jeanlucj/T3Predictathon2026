...
resumable: a job that hits its limit loses only the evaluations in
flight. Queue several jobs instead, each picking up where the last left
off. Configurations already run are skipped by hash.
