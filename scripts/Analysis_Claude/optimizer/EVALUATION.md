...
- **→ returns** `ch` is `list(cfg, source, ei)`; `source` is one of `seed` / `replicate` / `random_init` / `acquisition`.
...
- *Target-domain **and scheme** scoping:* `evals.sqlite` is a **shared archive of every evaluation ever run**, across target domains *and CV schemes*. But a given optimization only wants evidence from **its own** domain and **its own scheme** (the premise: the best pipeline for a domain/scheme is what you deploy on a *new* trial from that domain, under that scheme). So `choose_config()` filters the store **first** — `filter_evals_to_universe` (the run's pinned trial ids, resolved once by `pin_trial_universe()` from `.apply_target_domain` on the catalogue) **then** `filter_evals_to_scheme(settings$optimize_scheme)` — scoping *everything* downstream: seeds, the done/avoid dedup set, aggregation, the surrogate, and the incumbent. A config evaluated only in *another* domain or *another* scheme therefore counts as **untried here** and gets (re)run to build in-slice evidence. Each row stores its `trial_id` and `scheme`, so both filters are pure offline predicates; membership is decided on the id, never on the trial's metadata, so a trial T3 later renames or re-attributes cannot move a stored row in or out of the slice. **CV0 and CV00 are distinct tasks** (they can have different optimal pipelines — the sim world even models it, `evaluate.R:147`), so the optimizer targets exactly **one** scheme per run (`optimize_scheme`); to optimize both, run twice against the same store/cache. In `simulate` mode the universe is NULL (no filtering), but the scheme filter still applies. **🚩 red flag:** a run with a narrow `target_domain`/scheme never leaves the `seed`/`random_init` phase because almost no stored rows are in-slice (expected early on — it is gathering in-slice data), or `report.md` shows a large "out of this scheme/domain" count you did not expect.
...
- **Tier 3 — store / data helpers / reporting.** `store_eval`↔`read_evals` round-trip incl. NA score and `detail`; the four domain columns persist and `filter_evals_to_universe` restricts to the run's pinned trial ids (NULL/empty = no constraint; a renamed trial's rows stay in); `run_id_for`/`record_run`/`run_universe` round-trip a run and its universe, and the first writer wins so every worker reads one universe; `.trial_backlog` drops a trial outside the universe or already computed, so nothing unsatisfiable can hold the replication gate open; `filter_evals_to_scheme` restricts to the target scheme and composes with the universe filter; `choose_config` re-offers a seed evaluated only outside the universe **or under another scheme** (and does *not* when it is in-slice); `optimizer_settings()` rejects an `optimize_scheme` outside `schemes`; `.matches_trait`/`.focal_trait_parts`/ `.apply_target_domain`; `.obs_tibble`+`get_observations` on a fake response; `cached` write/read; `report.R` (`format_config`, `method_importance`, `failure_summary`, "BEATS submissions" only when incumbent \> best seed).
