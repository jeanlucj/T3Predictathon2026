...
- **→ returns** `ch` is `list(cfg, source, ei)`; `source` is one of `seed` / `random_init` / `acquisition`.
...
- *No replication yet:* `done_hashes` gates every selection path, so a configuration is evaluated **exactly once** and no config can reach `incumbent_min_reps`. `incumbent_config()` falls back to ranking all scored configs, and the BLUP shrinkage in `aggregate_scores()` is what keeps a single lucky evaluation from taking the title. Deliberate replication across focal trials is `config_replication`, the next revision. **🚩 red flag:** the incumbent changes on nearly every checkpoint — the leader is being set by noise, which is the symptom replication is meant to remove.
